...
function getArtistByIndex(array,desiredIndex) {
  return `the artist at index ${desiredIndex} is ${array[desiredIndex].name}`
}
...
function get20s(array) {
  const bornAndDied20s = []
  for (let i = 0; i < array.length; i++) {
    const yearsofBirthAndDeath = array[i].years.split(' ');
    const born = yearsofBirthAndDeath[0]
    const died = yearsofBirthAndDeath[2]
...
function addArtist(array) {
  array.push(
    { 
      "id": 20,
      "name": "Simone Taarnskov", 
      "years": "1990 - current day",
      "genre": "Web Design", 
      "nationality": "American",
      "bio": "Nam ratione fuga possimus. Suscipit perspiciatis labore beatae velit. Consectetur recusandae doloremque corporis.",
    } 
  )
  return array
}
...
function lotsOfArt(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    const worksCount = array[i]["paintings"];
    if (worksCount > 100) {
...
console.log(artistInfo(artists,'Francisco Goya'))
...
function artistByCountry(array,artistNationality){
  const artistListByCountry = []
  for (let i = 0; i < array.length; i++) {
    if (array[i].nationality === artistNationality) {
      artistListByCountry.push(array[i].name)
    }
    
  }
  return artistListByCountry;
}
